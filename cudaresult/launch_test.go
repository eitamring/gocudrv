...
func TestLaunchCooperativeKernel(t *testing.T) {
	value := uint32(7)
	params := []unsafe.Pointer{unsafe.Pointer(&value)}
	...
	cases := []struct {
		...
		{"nil driver", nil, ErrNotInitialized},
		{"symbol unavailable", &cudasys.Driver{}, ErrSymbolUnavailable},
		{
			"success",
			&cudasys.Driver{CuLaunchCooperativeKernel: func(
				fn cudasys.CUfunction,
				gridX, gridY, gridZ uint32,
				blockX, blockY, blockZ uint32,
				sharedMemBytes uint32,
				...
				if sharedMemBytes != 8 || stream != 9 {
					t.Errorf("shared/stream = %d/%#x, want 8/9", sharedMemBytes, stream)
				}
				if gotParams == nil {
					t.Fatal("nil kernel params")
				}
				if got := unsafe.Slice(gotParams, 1); *(*uint32)(got[0]) != value {
					t.Errorf("arg0 = %d, want %d", *(*uint32)(got[0]), value)
				}
				return cudasys.CUDA_SUCCESS
			}},
		...
		},
		{
			...
			&cudasys.Driver{CuLaunchCooperativeKernel: func(
				...
				*unsafe.Pointer,
				...
				return cudasys.CUDA_ERROR_COOPERATIVE_LAUNCH_TOO_LARGE
			}},
			ErrCooperativeLaunchTooLarge,
		},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			err := LaunchCooperativeKernel(tc.driver, 0xCAFE, 2, 3, 4, 5, 6, 7, 8, 9, &params[0])
			if tc.wantErr == nil {
				if err != nil {
					t.Errorf("unexpected err: %v", err)
				}
