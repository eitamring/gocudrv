...
func TestGlobalReadWriteCanceledBeforeSubmit(t *testing.T) {
	ctx, mod := globalTestModule(t)
	g, err := mod.Global("counter")
	if err != nil {
		t.Fatalf("Global: %v", err)
	...
	ctx.driver.CuMemcpyHtoD = func(cudasys.CUdeviceptr, *byte, uint64) cudasys.CUresult {
		t.Error("HtoD must not run when context is already canceled")
		return cudasys.CUDA_SUCCESS
	}
	ctx.driver.CuMemcpyDtoH = func(*byte, cudasys.CUdeviceptr, uint64) cudasys.CUresult {
		t.Error("DtoH must not run when context is already canceled")
		return cudasys.CUDA_SUCCESS
	}
	canceled, cancel := context.WithCancel(context.Background())
	cancel()
	if err := WriteGlobal(canceled, g, []float32{1, 2, 3, 4}); !errors.Is(err, context.Canceled) {
		t.Errorf("WriteGlobal err = %v, want context.Canceled", err)
	}
	if err := ReadGlobal(canceled, make([]float32, 4), g); !errors.Is(err, context.Canceled) {
		t.Errorf("ReadGlobal err = %v, want context.Canceled", err)
	}
}

func TestGlobalReadWritePropagatesDriverError(t *testing.T) {
	ctx, mod := globalTestModule(t)
	g, err := mod.Global("counter")
	if err != nil {
		t.Fatalf("Global: %v", err)
	...
	if err := WriteGlobal(context.Background(), g, []float32{1, 2, 3, 4}); !errors.Is(err, ErrInvalidValue) {
		t.Errorf("WriteGlobal err = %v, want ErrInvalidValue", err)
	}
	if err := ReadGlobal(context.Background(), make([]float32, 4), g); !errors.Is(err, ErrInvalidValue) {
		t.Errorf("ReadGlobal err = %v, want ErrInvalidValue", err)
	}
}

func TestGlobalNilReceiverAccessors(t *testing.T) {
	var g *Global
	if g.Bytes() != 0 {
		t.Errorf("nil Bytes = %d, want 0", g.Bytes())
	}
	if g.Name() != "" {
